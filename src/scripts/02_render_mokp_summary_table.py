#!/usr/bin/env python3
"""Convert `mokp_result.csv` into a LaTeX table.

Expected input: CSV produced by `src/scripts/01_summarize_mokp_runs.py`.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert mokp_result.csv into a LaTeX table."
    )
    parser.add_argument(
        "--csv",
        default=None,
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="Path to write LaTeX (default: results/mokp_result.tex).",
    )
    parser.add_argument(
        "--caption",
        default="Results on the MOKP across all problem sizes.",
        help="LaTeX caption text.",
    )
    parser.add_argument(
        "--label",
        default="tab:mokp_all_sizes",
...
def group_rows(rows):
    ...
    for row in rows:
        key = (int(row.get("n_objs", 0)), int(row.get("n_items", 0)))
    ...
    def row_sort_key(row):
        ...
        n_init_key = (
            int(n_init)
            if (method == "bpo" and n_init and n_init.isdigit())
            else 10**9
        )
...
def latex_tabular(rows, title=None, highlight_best=True):
    ...
    for group_idx, ((n_objs, n_items), group_rows_) in enumerate(groups):
...
def main():
    ...
    out_path = (
        ...
        if args.out
        else results_dir / "mokp_result.tex"
    )
