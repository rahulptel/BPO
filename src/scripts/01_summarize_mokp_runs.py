#!/usr/bin/env python3
"""Summarize final MOKP hypervolume results from the outputs directory."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect final MOKP hypervolume results from outputs."
    )
    parser.add_argument(
        "--outputs-dir",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=26,
        help="Minimum iseed to include (default: 26).",
    )
    parser.add_argument(
        "--iseed-max",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["100", "150"],
        help="BPO n_init values to include (default: 100 150).",
    )
    parser.add_argument(
        "--save-csv",
        default=None,
        help="Optional path to write a CSV summary (default: results/mokp_result.csv).",
    )
    parser.add_argument(
        "--save-run-csv",
        default=None,
        help="Optional path to write per-run MOKP results (default: results/mokp_result_runs.csv).",
    )
...
def _matches_bpo_n_init(cfg_sig, bpo_n_init):
    if not bpo_n_init:
        return True
    for value in bpo_n_init:
        if f"n_init-{value}" in cfg_sig:
            return True
    return False
...
def main():
    ...
    for n_objs, n_items, time_limit in MOKP_OBJ_ITEMS_TIME:
        allowed_specs[(n_objs, n_items)].add(int(time_limit))
    ...
    for method in args.methods:
        ...
        for run_path in run_files:
            desc = _parse_mokp_descriptor_from_path(run_path)
            if desc is None:
                unmatched += 1
                continue
            ...
            if method == "bpo" and not _matches_bpo_n_init(cfg_sig, args.bpo_n_init):
                continue
            ...
            key = (desc["objs"], desc["items"], cfg_sig)
            bucket = grouped[key]
            ...
            if method != "ksa" and rseed is not None:
                bucket["rseeds"].add(int(rseed))

        if not grouped:
            continue

        for n_objs, n_items, cfg_sig in grouped.keys():
            data = grouped[(n_objs, n_items, cfg_sig)]
            ...
            missing_rseeds = []
            if method != "ksa" and expected_rseeds is not None:
        ...
        if unmatched:
            print(
                f"(skipped {unmatched} {method} json files not matching mokp descriptor)"
            )
    ...
    if not rows:
        ...
        for row in rows:
            n_objs = row["n_objs"]
            n_items = row["n_items"]
            if (n_objs, n_items) != (current_obj, current_items):
                current_obj, current_items = n_objs, n_items
                print(f"\n-- objs={n_objs} items={n_items} --")
            print(
    ...
    if args.save_csv:
        ...
        csv_path = results_dir / "mokp_result.csv"
    if csv_path:
    ...
    if args.save_run_csv:
        ...
        csv_path = results_dir / "mokp_result_runs.csv"
    if csv_path:
